"""Recommendation page — set your priorities, get the best country and place to stay."""
...
st.set_page_config(page_title="Recommendation · Holiday Planner", layout="wide")
apply_styles()
...
st.write("Tell us what matters most for your trip. Adjust the sliders and the ranking updates instantly.")
...
weights = {
    ...
    "amenity_score": st.sidebar.slider("Restaurants and attractions", 0.0, 1.0, 0.20, 0.05),
...
# ---- ranking chart ----
st.subheader("Ranked destinations by your priorities")
...
with right:
    ...
    if not (top.get("selected_accommodation_rating") != top.get("selected_accommodation_rating")):
        bits.append(f"Rated {top['selected_accommodation_rating']:.1f}")
    st.write(" · ".join(bits) if bits else "Not listed")
